...
void BaseLayout::m_resetNodeStates() {
    if (!m_nodes.empty()) {
        ...
        for (auto node : m_nodes) {
            ...
            if (node_ptr != nullptr) {
                auto &node_datas = node_ptr->getDatasRef<BaseNode::BaseNodeDatas>();
                node_datas.layout.cell = ez::ivec2(-1);  // -1 pour que le 1er node (genre 'main') soit mit dans la colonne 0, sinon il serait ignor�
                node_datas.layout.inserted = false;  // pour voir si on insere le node plusieurs fois (utilit� que pour debug)
                node_datas.layout.used = false;  // node utiisi� ou non, pour cacher les nodes qui servent a rien
            }
        }
    }
}
